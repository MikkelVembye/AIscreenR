...
extract_info <- function(selector){

  row <- webpage |> rvest::html_nodes(selector)
  ...
  Output_price <- Output_price[-1]

  Input_price_d <- dplyr::if_else(stringr::str_detect(Input_price, "M tok"), 1e6, NA_real_)
  Output_price_d <- dplyr::if_else(stringr::str_detect(Input_price, "M tok"), 1e6, NA_real_)
  ...
  tibble::tibble(Model = models, Input = Input_price, Output = Output_price)

}

price_table <- function(){

  selector1 <- "#gpt-4-turbo > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > table:nth-child(1) > tbody:nth-child(1) > tr"
  selector2 <- "#gpt-4 > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > table:nth-child(1) > tbody:nth-child(1) > tr"
  selector3 <- "#gpt-3-5-turbo > div:nth-child(1) > div:nth-child(1) > div:nth-child(2) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > table:nth-child(1) > tbody:nth-child(1) > tr"

  selectors <- c(selector1, selector2, selector3)

  price_tab <- purrr::map(selectors, ~ extract_info(.x)) |> purrr::list_rbind()
  price_tab
