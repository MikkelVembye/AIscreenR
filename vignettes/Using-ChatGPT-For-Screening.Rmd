...
result_object$answer_data_sum |>
  summarise(
    ...
    n_screened = n(),
    n = n_screened - n_missing,
    n_false_ex = sum(if_else(human_code == 1 & final_decision_gpt_num == 0, 1, 0, missing = NA), na.rm = TRUE),
    n_false_in = sum(if_else(human_code == 0 & final_decision_gpt_num == 1, 1, 0, missing = NA), na.rm = TRUE),
    ...
    accuracy = (n_false_in + n_false_ex)/n,
    ...
    pe = 1/n * ((cm1*rm1)/n + (cm2*rm2)/n),
    
    
    kappa = (p_agreement - pe)/(1-pe),
    
    
    SE_kappa = sqrt( (p_agreement*(1-p_agreement)) / n*(1-pe)^2),
