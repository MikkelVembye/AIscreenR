...
```{r}
word_path1 <-  system.file("extdata", "word_pormpt_1.docx", package = "AIscreenR")

prompt <- 
  readtext::readtext(word_path1)$text |> 
...
models <- c("gpt-3.5-turbo-0613", "gpt-4")
reps <- c(10, 1)
rpm <- models_rate_limits$requests_per_minute

plan(multisession)

result_object <- 
  tabscreen_gpt(
    data = filges2015_dat, # ris-file data generated with 
...
#> * The approximate price of the current (simple) screening will be around $7.6544.
#> * Consider removing references that has no abstract since these can distort the accuracy of the screening
#>  Progress: ──────────────────────────────────────────────────────────────────────────────────────────── 100%
#>  * NOTE: Requests falied for 12 titles and abstracts."
...
result_object$answer_data_sum |>
  summarise(
    n_missing = sum(is.na(final_decision_gpt_num)),
    n_false_in = sum(if_else(human_code == 1 & final_decision_gpt_num == 0, 1, 0, missing = NA), na.rm = TRUE),
    n_false_ex = sum(if_else(human_code == 0 & final_decision_gpt_num == 1, 1, 0, missing = NA), na.rm = TRUE),
    n_true_in = sum(if_else(human_code == 1 & final_decision_gpt_num == 1, 1, 0, missing = NA), na.rm = TRUE),
    n_true_ex = sum(if_else(human_code == 0 & final_decision_gpt_num == 0, 1, 0, missing = NA), na.rm = TRUE),
   
    accuracy = 1- (n_false_in + n_false_ex)/n(),
    
    sensitivity = n_true_in / (n_true_in + n_false_in),
    
    specificity = n_true_ex / (n_true_ex + n_false_ex),
    
    .by = c(promptid, model, top_p) 
  ) |> 
  print(width = 200)
