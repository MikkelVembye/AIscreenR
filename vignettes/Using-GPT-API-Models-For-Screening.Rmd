...
You are now ready to run the full screening! In this example, it can be conducted by substituting the `test_dat` with `all_dat` in the code used to screen the test data. 
...
To steer the screening process when having a large collection of RIS-files, it can be advantageous to split the full data into minor batches. For example one could screen batches of 1000-2000 references at a time. `base::split()` can be used for this purpose. 
